{{!----
Description : 
Author : Gordon Truslove
Date : 10/18/2019, 10:51:37 AM
----}}
<div data-view="Global.BackToTop"></div>
<div class="ft-footer-links">
	<div class="ft-footer-links-inner">

<div id="banner-footer1" class="footer-content-col2">
		<div data-cms-area="global_banner_footer1" data-cms-area-filters="global">
		{{!----<h4>Footer Header</h4>
		<h5>Footer Sub Header</h5>
		<a>Footer Link 1</a>
		<a>Footer Link 2</a>
		<a>Footer Link 3</a>
		<h4><a>Footer Header Link</a></h4>
		<h5><a>Footer Sub Header</a></h5>
		----}}
		</div>
	</div>
	<div id="banner-footer2" class="footer-content-col2">
		<div data-cms-area="global_banner_footer2" data-cms-area-filters="global">
		
		</div>
	</div>
	<div id="banner-footer3" class="footer-content-col2">
		<div data-cms-area="global_banner_footer3" data-cms-area-filters="global">
		
		</div>
	</div>
	<div id="banner-footer4" class="footer-content-col2">
		<div data-cms-area="global_banner_footer4" data-cms-area-filters="global">
	
		</div>
	</div>
</div>
</div>
<div class="footer-content">

	
	<div><img class="ft-footer-logo" src="{{getExtensionAssetsPath 'img/FooterLogo2015.svg'}}" alt="Flexitog Icon"></div>
                    <a href="{{translate '/privacy'}}">{{translate 'Privacy Policy'}}</a> <span class="ft-footer-link-break">|</span> <a href="{{translate '/termsandconditions'}}">Terms &amp; Conditions</a> <span class="ft-footer-link-break">|</span> <a href="{{translate '/cookiepolicy'}}">{{translate 'Cookie Policy'}}</a>
</div>


{{!----
Use the following context variables when customizing this template:

	showFooterNavigationLinks (Boolean)
	footerNavigationLinks (Array)
	
----}}

